import React, { useRef, useState } from "react";
import { motion } from "framer-motion";
import emailjs from "@emailjs/browser";

import { styles } from "../styles";
import { EarthCanvas, CV } from "./canvas";
import { SectionWrapper } from "../hoc";
import { slideIn } from "../utils/motion";

const Contact = () => {
  const formRef = useRef();
  const [form, setForm] = useState({
    name: "",
    email: "",
    message: "",
  });

  const [loading, setLoading] = useState(false);

  const handleChange = (e) => {
    const { target } = e;
    const { name, value } = target;

    setForm({
      ...form,
      [name]: value,
    });
  };

  const handleSubmit = (e) => {
    e.preventDefault();
    setLoading(true);

    emailjs
      .send(
        "service_ph4vpse",
        "template_rfy6adj",
        {
          from_name: form.name,
          to_name: "Daniel",
          from_email: form.email,
          to_email: "daniro1498@gmail.com",
          message: form.message,
        },
        "noqaN68KRiN9oa7ar"
      )
      .then(
        () => {
          setLoading(false);
          alert("Thank you! I will get back to you as soon as possible");

          setForm({
            name: "",
            email: "",
            message: "",
          });
        },
        (error) => {
          setLoading(false);
          console.error(error);

          alert("Ahh, something went wrong :( Please try again");
        }
      );
  };

  return (
    <div
      className={`xl:mt-12 flex xl:flex-row flex-col-reverse gap-10 overflow-hidden`}
    >
      <motion.div
        variants={slideIn("left", "tween", 0.2, 1)}
        className="flex-[0.75] p-8 rounded-2xl"
      >
        <p className={styles.sectionSubText}>Ready for takeoff?</p>
        <h3 className={styles.sectionHeadText}>Contact</h3>
        <CV />

        <form
          ref={formRef}
          onSubmit={handleSubmit}
          className="mt-12 flex flex-col gap-8"
        >
          <label className="flex flex-col">
            <span className="text-white font-medium mb-4">Your Name</span>
            <div className="green-pink-gradient w-full p-[1px] rounded-[10px]">
              <input
                type="text"
                name="name"
                value={form.name}
                onChange={handleChange}
                className="bg-tertiary w-full py-4 px-6 placeholder:text-white text-white outline-none font-medium rounded-[10px]"
              />
            </div>
          </label>
          <label className="flex flex-col">
            <span className="text-white font-medium mb-4">Your email</span>
            <div className="green-pink-gradient w-full p-[1px] rounded-[10px]">
              <input
                type="email"
                name="email"
                value={form.email}
                onChange={handleChange}
                className="bg-tertiary py-4 px-6 placeholder:text-white text-white w-full rounded-[10px] outline-none font-medium"
              />
            </div>
          </label>
          <label className="flex flex-col">
            <span className="text-white font-medium mb-4">Your Message</span>
            <div className="green-pink-gradient w-full p-[1px] rounded-[10px]">
              <textarea
                rows={7}
                name="message"
                value={form.message}
                onChange={handleChange}
                placeholder="Tell me everything!"
                className="bg-tertiary py-4 px-6 -mb-1.5 placeholder:text-white text-white w-full rounded-[10px] outline-none font-medium"
              />
            </div>
          </label>
          <div className="GradientBorderv2 w-fit p-[1px] rounded-[10px]">
            <button
              type="submit"
              className="bg-tertiary py-3 px-8 mb-0.5 rounded-[10px] outline-none w-fit text-white font-bold shadow-md shadow-primary"
            >
              {loading ? "Sending..." : "Send  🚀"}
            </button>
          </div>
        </form>
      </motion.div>

      <motion.div
        variants={slideIn("right", "tween", 0.2, 1)}
        className="xl:flex-1 xl:h-auto md:h-[550px] h-[350px]"
      >
        <EarthCanvas />
      </motion.div>
    </div>
  );
};

export default SectionWrapper(Contact, "contact");
